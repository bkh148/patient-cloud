...
def dashboard():
    ...
    try:
        ...
        ).get_appointments_created_by(session['user']['user_id'])
        metadata['patients'] = services.user_service(
        ).get_all_users_patients(session['user']['user_id'])
        user_ids = [patient['user_id'] for patient in metadata['patients']]
        metadata['online_users'] = {user_id: online_patients[user_id]
                                    for user_id in user_ids if user_id in online_patients}
        metadata['locations'] = services.location_service().get_all_locations()

        metadata['user_roles'] = services.user_service(
        ...
        metadata['templates']['patient_item'] = render_template(
            'clinician/patient_container_item.html')
        ...
        metadata['templates']['appointments_item'] = render_template(
            'clinician/appointment_container_item.html')
    ...
    return render_template('clinician/index.html', title='Dashboard - Clinician',
                           ...
                           style_paths=['css/appointment.css',
                                        'css/patients.css'],
                           script_paths=[
                               'js/appointment.js', 'js/invite.js', 'js/patients.js', 'js/main.js'],
